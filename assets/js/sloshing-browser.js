(function () {
  ...
  function label(c) {
    ...
    if (c.loading_condition === 'stationary') return 'Stationary no-roll numerical baseline';
  ...
  function renderCard(c) {
    ...
    var links = '<a class="primary" href="' + caseUrl + '">Open case</a>' + (c.depth === 'simple' ? '' : '<a href="' + qaUrl + '">QA detail</a>');
    ...
    var videoTruth = c.video ? (c.fullCycleVideo ? ' Video: 1080p complete final forcing cycle; no temporal interpolation.' : ' Video: 1080p retained final window; not a full-cycle field record.') : '';
    return '<article class="case-card"><div class="case-media' + (c.image ? '' : ' no-media') + '">' + imageMarkup + '</div><div class="case-body"><div class="badges"><span class="badge ' + c.depth + '">' + title(c.depth) + '</span><span class="badge">' + title(c.loading_condition) + '</span><span class="badge' + (c.qa.indexOf('exceed') >= 0 || c.qa.indexOf('warn') >= 0 || c.qa.indexOf('caveat') >= 0 ? ' qa-warning' : '') + '">' + title(c.qa) + '</span></div><h2>' + safe(c.title || c.case_id) + '</h2><p class="case-subtitle"><code>' + safe(c.case_id) + '</code> · ' + safe(c.summary || label(c)) + '</p><div class="case-facts"><div><span>Mesh</span><strong>' + safe(title(c.mesh)) + '</strong></div><div><span>Period / frequency</span><strong>' + safe(period) + '</strong></div><div><span>Cells</span><strong>' + safe(c.mesh_cells || '—') + '</strong></div><div><span>Media</span><strong>' + safe(media) + '</strong></div></div><p class="media-truth"><strong>' + truth + '.</strong>' + videoTruth + '</p><div class="case-links">' + links + '</div></div></article>';
  }
  ...
  fetch(manifestUrl, { credentials:'same-origin' }).then(function (r) { if (!r.ok) throw new Error('Manifest HTTP ' + r.status); return r.json(); }).then(function (manifest) {
    ...
    var cases=loaded.sets[0], previews=loaded.sets[1], inputs=loaded.sets[2], mesh=loaded.sets[3], series=loaded.sets[4], derived=loaded.sets[5], audits=loaded.sets[6], caseCatalog=loaded.sets[7], mediaMetadata=loaded.sets[8];
    catalog = cases.map(function (c) { var published=caseCatalog.find(function (x) { return x.case_id === c.case_id; }) || {}; var directImage=mediaFor(previews,c.case_id,'image/'); var image=directImage || (published.representative_case_id ? mediaFor(previews,published.representative_case_id,'image/') : null); var video=mediaFor(previews,c.case_id,'video/'); var videoMeta=video && mediaMetadata.find(function (m) { return m.preview_id === video.preview_id; }); var mediaTruth=directImage ? 'case_specific' : (image ? 'representative' : 'none'); return Object.assign({},c,published,{image:image,video:video,media_truth:mediaTruth,fullCycleVideo:!!(videoMeta && videoMeta.review_scope.indexOf('complete_final_cycle') === 0),depth:published.evidence_depth || depthFor(c,inputs,mesh,series,derived,image,video),qa:published.qa_summary || qaFor(audits,c),assetBase:loaded.base.href}); });
