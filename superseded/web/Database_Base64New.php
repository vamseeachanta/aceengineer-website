...
//$Password = isset($_REQUEST['Password']);
    ...
    if ($Mode == "SaveContent") {
       ...
       $result = mysql_query("INSERT INTO `ace_blog`(`BlogContent`,`BlogDate`,`Heading`,`UrlHeading`) VALUES ('$Content',curdate() ,'$HeadingTitle' ,'$UrlHeadingTitle' )");
    ...
    if ($Mode == "UpdateMode") {
        ...
        $SelectedHeading = mysql_real_escape_string($_REQUEST['SelectedHeading']);
        $BlogHeading = mysql_real_escape_string( $_REQUEST['BlogHeading'] );
        $BlogUrlHeading = mysql_real_escape_string( $_REQUEST['BlogUrlHeading'] );
       $result = mysql_query("Update  `ace_blog` set `BlogContent` ='$UpdateContent' ,`Heading` ='$BlogHeading' ,`UrlHeading` ='$BlogUrlHeading' where  `Heading`='$SelectedHeading' ");
    ...
    if ($Mode == "DeleteMode") {
       ...
       $result = mysql_query("Delete  From `ace_blog` where  `Heading`='$SelectedHeading' ");
    ...
    $result = mysql_query("SELECT `Heading` FROM `ace_blog`");
    $result_set=array();
    $arr = array();
...
if ($Mode == "GetSelectedContent") {
   $Heading = mysql_real_escape_string($_REQUEST['Heading']);
    $result = mysql_query("SELECT `BlogContent`,`Heading`,`UrlHeading` FROM `ace_blog` where `Heading`= '$Heading' ");
    $result_set=array();
    $arr = array();
    while ($row = mysql_fetch_array($result)) {
