...
var FundamentalIndicatorHandler = {
    ...
    {        
        ...
        function addTicker(ticker)
        {
            ...
            ReturnAssessmentHandler.addNewTickerToCompare(tName, true,function(){
                alert("Ticker Add in Other Tad Also");
            });
        }
        ...
        $(that.benchMark).change(function()
        {
            ...
            {                
                ...
                if(isChecked)
                {
                    cols = [0,1,4,5,6,7,8,9,10];
                }
                else{
                    cols = [0,1,4,5,6,7,8,9];
                }
    ...
    applyMask: function(pDiv,state)
    {
        ...
        {      
            ...
            $(this.maskDiv).html("<table style='width:100%;height:100%'><tr><td style='vertical-aling:middle;text-align:center'><img src='res/images/334.gif' /></td></tr></table>");
     ...
     */
    drawChart: function(pChart,pChartDataView,pTitle,pXtitle,pYtitle,drawMode)
    {        
    ...
    {           
        ...
        doAjax("http://ichart.finance.yahoo.com/table.csv?"+query, "", function(data){
            ...
            bB = StockAnalysis.getBollingerBands(rates,ma[3].length-1);
    ...
    {   
        ...
        try{
            StockData.loadTickerSummary(ticker, function(data){
                $(summaryDiv).append(StockData.makeSummaryDiv(data));
        ...
        function myHandler(data)
        {
            ...
            try
            {
                ...
                var ma50 = ma[3];
                var EMA = ma[4];
                var bB = reverseArray(ma[5]);
                var lB= reverseArray(ma[8]);
                var uB= reverseArray(ma[9]);
                var ext_lB= reverseArray(ma[10]);
                var ext_uB= reverseArray(ma[11]);
                ...
                var noCRintvls = 5;//number of chart recommendations intervals
                var partitions = Math.ceil(ma1.length/noCRintvls);
                
                var rcmds = [];//chart recommendations
               rcmds = ["1",,,,,,,,,,,,,,,"3"];
                ...
                for(var i=0;i<dates.length;i++,counter++)
                {
                    ...
                    rows[i] = [dates[i],ma1[i],ma5[i],ma13[i],ma50[i],EMA[i],lB[i],uB[i],ext_lB[i],ext_uB[i],bMa1[i],bMa5[i],bMa13[i],bMa50[i],rcmds[i]];
                }                
            
                dataTable.addRows(rows);
                var viewCols = [];
                dataView = new google.visualization.DataView(dataTable);
                viewCols = [0,1,4,5,6,7,8,9];
