...
function chartRecommendationsSummary(ema,ub,lb,eub,elb,tdMA,price){
    ...
    if((eub[0]>curr)&&(curr>ub[0])){
        recSummary+= "-<b style='color:orange;'> HOLD</b><br>- Price near the upper Bollinger band.<br>- Testing resistence at upper bollinger band. <br>- Expect price to move higher if fundamentals stay strong and no significant negative economic/political events happen.<br>";//rec5
    }
...
$(function(){
    ...
    $('#cprReport').click(function(){
        try
        {
            ...
            //cPS = cPS.textContent;
            var csText = [];
            //            csText[0] = $('#comparePortfolioSummary')[0].textContent.toString();
            csText[0] = $('#comparePortfolioSummary')[0].textContent.toString();
            
            var cS = csText[0].split("•");
            //            var cS = csText.split("-");
            ...
            //            alert(cSummary.toString());
            //            cSummary = cSummary.toString();
            ...
            xml.onreadystatechange = function()
            {
                ...
                /* var d = document.createElement("a");
                    ...
                    alert($("<a href=\""+src+"\">Download Report</a>"));
                    $("<a href=\""+src+"\" target='blank'>Download Report</a>").click();
                    $(d).click(function(){
                        alert("clicked");
                    });
                    $(d).click();*/
            ...
            xml.send("size="+noOfTickers+"&images="+cImg+"&cP="+cCurr+"&cR="+cRcmds+"&cN="+cTickers+"&chartSummary="+cSummary+"&cPImgD="+addComChartData);
...
function drawChart() 
{
    try
    {
        ...
        if(($("#compChart")[0])!=null){
            var CompLChart =  new google.visualization.LineChart($("#compChart")[0]);
            CompLChart.draw(data,lineChartOptions);
        }
        ...
        portRec = portRec.toFixed(2);
       
        cnt +=" is  "+portRec+" percent while the market ";
        cnt += marketRec<0?"<span style='color:red;'><b>Loss</b></span>":"<span style='color:green;'><b>Gain</b></span>";
        marketRec = marketRec.toFixed(2);
        cnt += " is "+marketRec+" for the week. "
        ...
        var mar = (Math.abs(marketRec)-Math.abs(portRec)).toFixed(2);
        cnt += "(by "+mar+ ") than the Market Returns.";
